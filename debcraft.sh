...
display_help() {
  cat << EOF
usage: debcraft <build|improve|test|release|shell|prune|logs> [options] [<path|pkg|srcpkg|dsc|git-url>]
...
do
  case "$1" in
    ...
    -*)
      ...
      exit 1
      ;;
    build | improve | test | release | shell | prune | logs)
      export ACTION="$1"
      shift
      ;;
    *)
...
then
  ...
  log_error "Argument '$TARGET' not one of <build|improve|test|release|shell|prune|logs>"
  echo
  display_help
  exit 1
fi
